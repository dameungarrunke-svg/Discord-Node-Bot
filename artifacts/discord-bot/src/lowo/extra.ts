...
export async function cmdRename(message: Message, args: string[]): Promise<void> {
  // Renames a weapon by index → stored as suffix in id field
...
export async function cmdDismantle(message: Message, args: string[]): Promise<void> {
  ...
  const essenceGain = ({ common: 2, uncommon: 5, rare: 12, epic: 30, mythic: 80 } as const)[w.rarity] ?? 1;
  updateUser(message.author.id, (x) => {
...
export async function cmdBattlesetting(message: Message): Promise<void> {
  const u = getUser(message.author.id);
  await message.reply([
    "⚙️ **Battle Settings**",
    `Team size: 3 (fixed)`,
    `Your team: ${u.team.length === 0 ? "*empty*" : u.team.map(id => ANIMAL_BY_ID[id]?.name ?? id).join(", ")}`,
    `Modify with \`lowo team add/remove <animalId>\``,
  ].join("\n"));
}
