...
import {
  getMobilePlayers,
  LeaderboardPlayer,
  getMobilePinnedMessage,
  setMobilePinnedMessage,
  PinnedMessage,
} from "./mobileStore.js";
...
export function buildMobilePermanentPayload(): {
  content: string;
  embeds: EmbedBuilder[];
  files: AttachmentBuilder[];
} {
  const allPlayers = getMobilePlayers();
  const displayPlayers = allPlayers.slice(0, MAX_CARDS);
  const extra = allPlayers.length - displayPlayers.length;

  const now = Math.floor(Date.now() / 1000);

  let content = `**📱 TSB Top ${Math.min(allPlayers.length, MAX_CARDS)} Mobile Leaderboard**  •  Updated <t:${now}:R>`;
...
export async function refreshPinnedMobileLeaderboard(client: Client): Promise<void> {
  const pinned = getMobilePinnedMessage();
  if (!pinned) return;
  ...
  try {
    ...
    const payload = buildMobilePermanentPayload();
    await message.edit({
    ...
    console.error("Failed to refresh pinned mobile leaderboard:", err);
  }
}

export async function executeSetupMobileLeaderboard(
  ...
  const existing = getMobilePinnedMessage();
  if (existing && existing.channelId === channel.id) {
    ...
    if (old) {
      await interaction.editReply({
        content:
          `✅ A mobile leaderboard is already pinned in this channel.\n` +
          `Use \`/addmobileplayer\`, \`/editmobileplayer\`, or \`/removemobileplayer\` to update it automatically.`,
      });
      return;
    }
  }
  ...
  try {
    ...
    setMobilePinnedMessage(pinned);

    await interaction.editReply({
      content: `✅ Mobile Leaderboard deployed: ${msg.url}\n\nIt will auto-update whenever you add, edit, or remove players.`,
    });
  } catch (err) {
    console.error("Error setting up mobile leaderboard:", err);
    await interaction.editReply({
      content: "❌ Something went wrong while setting up the mobile leaderboard.",
    }).catch(() => {});
  }
}
