...
const MODULES: Record<string, { emoji: string; desc: string; commands: string[] }> = {
  ...
  ai: {
    ...
    commands: [
      "`mewo ai chatgpt <prompt>` — Chat with an AI assistant",
      "`mewo ai llama <prompt>` — Chat with LLaMA AI",
      "`mewo ai deepseek <prompt>` — Chat with DeepSeek AI",
      "`mewo ai imagine <prompt>` — Generate an image from a description",
      "`mewo ai perplexity <query>` — AI-powered web search with citations",
      "`mewo ai ocr` + image — Extract text from an image",
      "`mewo ai screenshot <url>` — Take a screenshot of any website",
      "`mewo ai download <url>` — Download media from YouTube, TikTok, and more",
      "`mewo ai tts openai <text>` — Convert text to speech",
      "`mewo ai tts elevenlabs <text>` — Convert text to speech (high quality)",
      "`mewo ai deepgeolocate <ip>` — Deep multi-source IP geolocation",
      "`mewo ai usage` — Check your daily AI usage",
    ],
  },
  roleplay: {
    emoji: "🎭",
